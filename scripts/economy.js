import { companions, defaultCosmetic, getHeroStats, getUpgradeCost, heroes, maxUpgradeLevel, starterHeroes } from "./config.js?v=coopstart2";
...
function renderHeroCard(state, hero, unlocked, selectable) {
  ...
  const status = unlocked ? `Stufe ${level}` : `${hero.price} M�nzen`;
...
function renderCompanionCard(state, companion) {
  ...
  const status = equipped ? "Aktiv" : owned ? "Ausr�sten" : `${companion.price} M�nzen`;
  return `
    <button class="fighter companion-card ${equipped ? "selected" : ""} ${owned ? "" : "locked"}" ${action} ${disabled ? "disabled" : ""}>
      ...
      <span class="stats">L�uft neben dir, keine St�rke</span>
      <span class="hero-status">${status}</span>
    </button>
  `;
}
...
function renderHeroDetails(state, dom) {
  ...
  dom.heroDetails.innerHTML = `
    <div>
      <span class="label">Ausgew�hlt</span>
    ...
    <button id="upgradeHeroBtn" ${maxed || state.coins < cost ? "disabled" : ""}>${maxed ? "Max Stufe" : `Upgrade ${cost} M�nzen`}</button>
  `;
}
