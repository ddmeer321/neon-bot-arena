...
test("legacy scores stay in normal while mode scores remain separate", () => {
  ...
  assert.equal(filterLeaderboard(scores, "one-heart")[0].score, 60);
});
...
test("leaderboards keep ten scores per mode instead of ten total", () => {
  const entries = normalizeLeaderboard([
    ...
    ...Array.from({ length: 12 }, (_, index) => ({ name: `H${index}`, score: 100 - index, wave: 1, mode: "one-heart" }))
  ]);
  ...
  assert.equal(filterLeaderboard(limited, "one-heart").length, 10);
});
